    ...
    SubShader {
        ...
        Pass {
            ...
            void gp(point v2g points[1], inout TriangleStream<g2f> triStream) {
                ...
                float wind = snoise(root.xyz * 0.35f + _Time.y * 0.25f) * 0.07f;
                float wind2 = snoise(root.xyz * 0.2f + _Time.y * 0.18f) * 0.04f;

                float3 p0 = root.xyz;
                float3 p1 = root.xyz + float3(
                    cx * curveMagnitude * 0.2 + wind * 0.3,
                    grassHeight * 0.33,
                    cz * curveMagnitude * 0.2 + wind2 * 0.3
                );
                float3 p2 = root.xyz + float3(
                    cx * curveMagnitude * 0.7 + wind * 0.7,
                    grassHeight * 0.66,
                    cz * curveMagnitude * 0.7 + wind2 * 0.7
                );
                float3 p3 = root.xyz + float3(
                    cx * curveMagnitude + wind,
                    grassHeight,
                    cz * curveMagnitude + wind2
                );
                ...
                for (i = 0; i < SEGMENTS; ++i) {
                    ...
                    triStream.Append(v[idx]);
                    triStream.Append(v[idx + 2]);
                    triStream.Append(v[idx + 1]);

                    triStream.Append(v[idx + 1]);
                    triStream.Append(v[idx + 2]);
                    triStream.Append(v[idx + 3]);
                }
            ...
            half4 fp(g2f i) : SV_Target {
                ...
                // Couleur du brin basée uniquement sur UV.y (stable, pas de normale)
                ...
                // === TOON LIGHTING STABLE ===
                // Utiliser une normale fixe (vers le haut) pour que le lighting ne bouge pas
                float3 stableNormal = float3(0, 1, 0);
                float NdotL = dot(stableNormal, normalize(_MainLightPosition.xyz)) * 0.5 + 0.5;

                // smoothstep au lieu de floor = pas de saut brusque
