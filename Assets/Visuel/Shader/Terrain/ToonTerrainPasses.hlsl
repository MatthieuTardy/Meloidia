...
// CORRECTION DE LA FONCTION RESPONSABLE DES PIXELS EN ESCALIER
half4 SplitMap(half4 map, half smoothness)
{
    // "s" détermine l'épaisseur de la transition pour effacer l'effet crénelé/pixel.
    // 0.01 = très fin (Toon mais propre), 0.5 = très large (mélange normal)
    half s = lerp(0.01, 0.5, smoothness);
    
    half4 res;
    res.r = smoothstep(0.1 - s, 0.1 + s, map.r - map.g - map.b - map.a);
    res.g = smoothstep(0.1 - s, 0.1 + s, map.g - map.r - map.b - map.a);
    res.b = smoothstep(0.1 - s, 0.1 + s, map.b - map.g - map.r - map.a);
    res.a = smoothstep(0.1 - s, 0.1 + s, map.a - map.g - map.b - map.r);
    
    // Normaliser les poids pour ne pas avoir de trous noirs
    half sum = max(res.r + res.g + res.b + res.a, 0.0001);
    res /= sum;

    // Mélanger avec le comportement normal d'Unity selon la valeur du slider
    return lerp(res, map, smoothness);
}
...
void SplatmapMix(float4 uvMainAndLM, float4 uvSplat01, float4 uvSplat23, inout half4 splatControl, out half weight, out half4 mixedDiffuse, out half4 defaultSmoothness, inout half3 mixedNormal)
{
    ...
    {
        half sharpness = lerp(50.0, 1.0, _TerrainBlendSmoothness);
        
        half4 opacityAsDensity = saturate((half4(diffAlbedo[0].a, diffAlbedo[1].a, diffAlbedo[2].a, diffAlbedo[3].a) - (1 - splatControl)) * sharpness);
    ...
    // APPEL DE NOTRE NOUVELLE FONCTION AVEC LE SLIDER D'OPACITÉ
    splatControl = SplitMap(splatControl, _TerrainBlendSmoothness);
...
#endif
{
    ...
    // Triplanar mapping pour le mur vertical
    ...
    // Mix du mur en fonction de l'inclinaison
    float slope = dot(inputData.normalWS, half3(0, 1, 0));
    float wallOpacity = smoothstep(_WallTextureCutoff, _WallTextureCutoff - _WallBlendSmoothness, slope);
    albedo = lerp(albedo, tri.rgb, wallOpacity);
    ...
    half d = dot(inputData.normalWS, mainLight.direction) * 0.5 + 0.5;

    half toonRamp = smoothstep(_ToonRampOffset, _ToonRampOffset + _ToonRampSmoothness, d);
    toonRamp = lerp(0.3, 1.0, pow(max(toonRamp, 0.001), _ToonContrast));

    toonRamp *= _LightIntensity;

    half shadowIntensity = lerp(1.0, mainLight.shadowAttenuation, _ShadowPower);
    half3 toonDiffuseLight = mainLight.color * toonRamp * shadowIntensity;

    half3 ambientLight = inputData.bakedGI * occlusion;

    half3 finalColor = albedo * (toonDiffuseLight + ambientLight);
